...
type DayStatus = 'checked' | 'missed' | 'today' | 'future';
...
// 模拟一周的数据
const INITIAL_WEEK_DATA: CheckInDay[] = [
  { date: '12.04', dayOfWeek: '周一', status: 'checked', points: 10, exp: 5, isMilestone: false },
  { date: '12.05', dayOfWeek: '周二', status: 'checked', points: 10, exp: 5, isMilestone: false },
  { date: '12.06', dayOfWeek: '周三', status: 'missed', points: 20, exp: 10, isMilestone: true, specialReward: '10% 优惠券' },
  { date: '12.07', dayOfWeek: '周四', status: 'today', points: 50, exp: 15, isMilestone: false }, // 今天奖励多一点
  { date: '12.08', dayOfWeek: '周五', status: 'future', points: 15, exp: 5, isMilestone: false },
  { date: '12.09', dayOfWeek: '周六', status: 'future', points: 15, exp: 5, isMilestone: false },
  { date: '12.10', dayOfWeek: '周日', status: 'future', points: 100, exp: 50, isMilestone: true, specialReward: '神秘盲盒' },
];
...
export const DailyCheckInPage: React.FC = () => {
  ...
  // State
  const [weekData, setWeekData] = useState<CheckInDay[]>(INITIAL_WEEK_DATA);
  ...
  const handleCheckIn = () => {
    ...
    // 模拟API调用
    setTimeout(() => {
      const todayIndex = weekData.findIndex(d => d.status === 'today');
      const updatedData = [...weekData];
      updatedData[todayIndex].status = 'checked';
      
      const todayReward = updatedData[todayIndex];
      ...
      // 更新数值
      setUserPoints(prev => prev + todayReward.points);
      setUserExp(prev => prev + todayReward.exp);
      
      // 显示弹窗
      setRewardMessage({ points: todayReward.points, exp: todayReward.exp, extra: '' });
                ...
                <div className="relative z-10">
                    ...
                    <div className="flex items-center gap-4 mb-6">
                        ...
                        <div>
                            ...
                            <div className="text-3xl font-bold font-mono text-white tracking-tight">{userPoints}</div>
                        </div>
                        {/* 兑换按钮 */}
                        <button 
                    ...
                    <div className="grid grid-cols-2 gap-3">
                         <div className="p-3 bg-white/5 rounded-lg border border-white/5">
                             ...
                             <div className="text-lg font-bold text-purple-400">{userCompute}</div>
                         </div>
                         <div className="p-3 bg-white/5 rounded-lg border border-white/5">
                             <div className="text-xs text-slate-500 mb-1 flex items-center gap-1"><Award size={10}/> 当前经验</div>
                             <div className="text-lg font-bold text-blue-400">Lv.12 <span className="text-sm text-slate-500 font-normal">({userExp})</span></div>
                         </div>
                    </div>
                </div>
             </div>
