...
import { Home, Search, Trophy, User, MessageSquare, Heart, Share2, Play , Sparkles} from 'lucide-react';
...
const CommunityPage = () => {
  ...
  // State for the Detail View (Comments/Interaction)
  const [selectedPost, setSelectedPost] = useState<Post | null>(null);
  
  // State for the Image View (Immersive View)
  ...
  const LandingPage = () => (
    <div className="min-h-screen bg-[#05050a] flex flex-col items-center justify-center relative overflow-hidden">
      ...
      <div className="z-10 text-center space-y-8 p-6 max-w-2xl">
        ...
        <button 
          onClick={() => setViewState(ViewState.APP)}
    ...
    <div className="flex h-screen w-full bg-[#05050a] text-white overflow-hidden relative">
      ...
      <nav className="w-16 md:w-20 bg-black/40 border-r border-white/5 flex flex-col items-center py-8 z-50 backdrop-blur-xl shrink-0">
        ...
        <div className="flex flex-col gap-8 w-full">
          <SidebarBtn 
            icon={<Home size={24} />} 
            isActive={activeTab === SidebarTab.HOME} 
            onClick={() => setActiveTab(SidebarTab.HOME)} 
            label="首页"
          />
          <SidebarBtn 
            icon={<Search size={24} />} 
            isActive={activeTab === SidebarTab.SEARCH} 
            onClick={() => setActiveTab(activeTab === SidebarTab.SEARCH ? SidebarTab.HOME : SidebarTab.SEARCH)} 
            label="搜索"
          />
          <SidebarBtn 
            icon={<Trophy size={24} />} 
            isActive={activeTab === SidebarTab.LEADERBOARD} 
            onClick={() => setActiveTab(activeTab === SidebarTab.LEADERBOARD ? SidebarTab.HOME : SidebarTab.LEADERBOARD)} 
            label="榜单"
          />
        </div>
        ...
        <div className="mt-auto">
          <SidebarBtn 
            icon={<User size={24} />} 
            isActive={activeTab === SidebarTab.PROFILE} 
            onClick={() => setActiveTab(SidebarTab.PROFILE)} 
            label="我的"
          />
        </div>
      </nav>

      {/* Expandable Panels (Search, Leaderboard) */}
      ...
      <main className="flex-1 relative overflow-y-auto scroll-smooth">
        ...
        <div className="sticky top-0 z-30 flex items-center justify-between px-6 py-4 bg-[#05050a]/80 backdrop-blur-md border-b border-white/5">
           <div className="flex gap-2">
             <button className="px-4 py-2 bg-white/10 rounded-full text-sm font-medium hover:bg-white/20 transition-colors">推荐</button>
             <button className="px-4 py-2 text-gray-400 text-sm font-medium hover:text-white transition-colors">关注</button>
             <button className="px-4 py-2 text-gray-400 text-sm font-medium hover:text-white transition-colors">最新</button>
           </div>
        ...
        {/* Masonry Grid Feed */}
        <div className="p-4 md:p-6 lg:p-8 pb-32"> {/* Added extra padding bottom for floating button */}
        ...
        <div className="fixed bottom-10 left-[calc(50%+2rem)] md:left-[calc(50%+2.5rem)] -translate-x-1/2 z-40 animate-in slide-in-from-bottom-10 fade-in duration-700 delay-300">
           <button 
            onClick={handleCreateClick}
           className="group relative flex items-center gap-3 px-8 py-3.5 bg-gradient-to-r from-purple-600 to-blue-600 rounded-full text-white font-semibold shadow-lg shadow-purple-500/40 hover:shadow-purple-500/60 hover:scale-105 active:scale-95 transition-all duration-300 ring-1 ring-white/20">
              {/* Glow Effect */}
      ...
      {/* Detail Modal Overlay (Comments) */}
      {selectedPost && (
        <DetailView post={selectedPost} onClose={() => setSelectedPost(null)} />
      )}
      
      {/* Image Viewer Overlay (Immersive) */}
      {viewingImage && (
        <ImageView post={viewingImage} onClose={() => setViewingImage(null)} />
      )}
