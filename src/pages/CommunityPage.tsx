...
const useMasonryColumns = (posts: Post[]) => {
  // 1. 仅将“列数”作为 State，因为它只随窗口变化，不随数据变化
  ...
  // 2. 核心：使用 useMemo 同步计算分列
  // 当 posts 变化时，这里会立即重新计算，并在同一次渲染周期中返回
  const columns = React.useMemo(() => {
    const newColumns: Post[][] = Array.from({ length: numCols }, () => []);
    
    posts.forEach((post, i) => {
      newColumns[i % numCols].push(post);
    });
...
const MasonrySkeleton = () => (
  <div className="columns-1 sm:columns-2 lg:columns-3 xl:columns-4 gap-6 space-y-6 animate-in fade-in duration-500">
    {Array.from({ length: 12 }).map((_, i) => (
      <div
        key={i}
        className="break-inside-avoid relative rounded-xl overflow-hidden bg-[#1a1a1a] border border-white/5"
      >
        {/* 模拟图片区域 */}
        <div
          className="w-full bg-slate-800/50 animate-pulse"
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="flex justify-between pt-2">
            <div className="h-6 w-6 rounded-full bg-slate-700/50 animate-pulse" />
...
const CommunityPage = () => {
  ...
  const masonryColumns = useMasonryColumns(displayPosts);
  // 分页相关状态
       ...
       {/* Feed Grid */}
        <div className="p-4 md:p-6 lg:p-8 pb-32">
          ...
          {isInitialLoading && displayPosts.length === 0 ? (
             ...
             /* 2. 真实数据列表：使用 Flex Masonry 物理分列布局 */
             <div className="flex gap-4 md:gap-6 items-start animate-in fade-in slide-in-from-bottom-4 duration-500">
                {masonryColumns.map((column, colIndex) => (
                  /* 每一列都是一个独立的垂直 Flex 容器 */
                  <div key={colIndex} className="flex-1 flex flex-col gap-4 md:gap-6 min-w-0">
                    {column.map((post) => {
                      // 判断是否是整个数据源的最后一个元素，用于触发无限加载
                      const isRealLast = post.id === displayPosts[displayPosts.length - 1]?.id;
                      
                      // 计算宽高比样式
                      const aspectRatioStyle = (post.width && post.height) 
                      ...
                      return (
                        <div 
                            key={post.id} 
                            // 绑定 ref 到最后一个元素
                            ref={isRealLast ? lastPostElementRef : null}
                            onClick={() => openImageView(post)}
                            className="relative group rounded-xl overflow-hidden bg-[#1a1a1a] cursor-pointer border border-white/5 hover:border-white/20 transition-all shadow-lg hover:shadow-purple-900/20 w-full"
                        >
                            {/* Image Area */}
                            <div 
                               className={`relative w-full bg-slate-800 overflow-hidden ${!aspectRatioStyle ? 'min-h-[200px]' : ''}`}
