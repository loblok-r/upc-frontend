...
import { 
  User, LogOut, ChevronRight, Zap, Award, 
...
export const UserMenu: React.FC<UserMenuProps> = ({ onLogout }) => {
  ...
  // ✅ 3. 鼠标移出处理：延迟关闭 (给用户 200ms 的时间移动鼠标跨过间隙)
    ...
    >
      ...
      <button className="flex items-center gap-3 py-1 focus:outline-none">
         <div className="text-right hidden md:block">
            ...
            <div className="text-[10px] text-amber-400 font-bold bg-amber-500/10 px-1.5 rounded inline-block border border-amber-500/20 mt-0.5">PRO USER</div>
         </div>
              ...
              <div className="space-y-4">
                 ...
                 <div className="space-y-1.5">
                    <div className="flex justify-between text-xs">
                       ...
                       <span className="text-amber-400 font-mono font-bold">{userData.points}</span>
                    </div>
                    <div className="h-1.5 w-full bg-black/40 rounded-full overflow-hidden">
                       <div className="h-full bg-gradient-to-r from-amber-500 to-orange-500" style={{ width: `${(userData.points / userData.maxPoints) * 100}%` }}></div>
                    </div>
                 </div>
              </div>
           </div>
